...
public class L3742_Maximum_Path_Score_in_Grid {
    public static void main(String[] args) {
        int[][] grid = {{0, 1}, {2, 1}};
        int k = 1;
        L3742_Maximum_Path_Score_in_Grid sol = new L3742_Maximum_Path_Score_in_Grid();
    ...
    private int helper(int r, int c, int[][] grid, int k) {
        if(r>=m || c>=n || k<0) return -1;  // invalid path
        int val = grid[r][c];
        boolean anyCost = val > 0;

        if(anyCost && k<=0) return -1;      // invalid path
